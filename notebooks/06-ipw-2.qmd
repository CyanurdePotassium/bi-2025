...
ipw_gee <- nonprob(selection = ~ size,         ## P(R_1 = 1 | ...)
                    ...
                    data = admin,               ## proba nielosowa S_A
                    method_selection = "logit", ## regresja logistyczna / probit / cloglog
                    control_selection = control_sel(est_method = "gee")) ## dzięki gee, wagi odtwarzają rozkład z próby (kalibrowany ipw)
...
Ćw. 1. 
Zastąpienie zmiennej size inną zmienną (nace lub region), porównanie wyników.
...
ipw_mle_region <- nonprob(selection = ~ region,
                          target = ~ single_shift,
                          svydesign = jvs_svy,
                          data = admin,
                          method_selection = "logit")

ipw_gee_region <- nonprob(selection = ~ region,
                          target = ~ single_shift,
                          svydesign = jvs_svy,
                          data = admin,
                          method_selection = "logit",
...
To samo, co wyżej:
```{r}
ipw_mle_region <- update(ipw_mle, selection = ~ region)
ipw_gee_region <- update(ipw_gee, selection = ~ region)
```
...
```{r}
summary(weights(ipw_mle_region))
summary(weights(ipw_gee_region))
```

```{r}
rbind(
  extract(ipw_mle_region),
  extract(ipw_gee_region)
)
```

```{r}
check_balance(~size-1, ipw_mle_region)
check_balance(~size-1, ipw_gee_region)
```
...
```{r}
check_balance(~size-1,ipw_mle2)$balance
check_balance(~size-1,ipw_gee2)$balance
```
Ćw. 2.
Size i private oraz ich interakcja
oszacowanie parametrów
porównanie wyników
rozkłady wag

$$
P(R_A = 1 | size, private, size+private)
$$

```{r}

ipw_mle3 <- nonprob(selection = ~ size + private + size * private,
                    target = ~ single_shift,
                    svydesign = jvs_svy,
                    data = admin,
                    method_selection = "logit")

ipw_gee3 <- nonprob(selection = ~ size + private + size * private,
                    target = ~ single_shift,
                    svydesign = jvs_svy,
                    data = admin,
                    method_selection = "logit",
...
```{r}
summary(weights(ipw_mle3))
summary(weights(ipw_gee3))
```

```{r}
plot(x = weights(ipw_mle3),
     y = weights(ipw_gee3),
     xlab = "Wagi IPW", ylab = "Wagi GEE")
```
